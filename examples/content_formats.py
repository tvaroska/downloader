    ...
    for format_name, format_config in CONTENT_FORMATS.items():
        if format_name in results and results[format_name].success:
            ...
            if format_name == "pdf" and result.content_size > 0:
                ...
                print(f"      JSON includes metadata - ideal for programmatic processing")
            elif format_name == "markdown" and result.content_size > 0:
                print(f"      Markdown preserves structure - good for documentation")
            elif format_name == "text" and result.content_size > 0:
                print(f"      Plain text is clean - best for content analysis")
    
    # Recommendations
    print(f"\n🎯 Recommendations:")
    
    if "text" in results and results["text"].success:
        print(f"   📝 Use text/plain for: Content analysis, search indexing, NLP processing")
    
    if "markdown" in results and results["markdown"].success:
        print(f"   📝 Use text/markdown for: Documentation, structured content, CMS integration")
    
    if "html" in results and results["html"].success:
        print(f"   🌐 Use text/html for: Web scraping, style preservation, full content")
    
    if "json" in results and results["json"].success:
        print(f"   📊 Use application/json for: API integration, metadata analysis, structured processing")
    
    if "pdf" in results and results["pdf"].success:
        print(f"   📄 Use application/pdf for: Document archival, printing, visual preservation")
