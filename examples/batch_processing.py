...
def print_batch_summary(batch_data: Dict[str, Any], batch_name: str):
    ...
    for i, result in enumerate(batch_data["results"], 1):
        ...
        if result.get("content") and len(result["content"]) < 100:
            preview = result["content"][:97] + "..." if len(result["content"]) > 97 else result["content"]
...
async def demonstrate_high_concurrency():
    ...
    high_concurrency_urls = [
        {"url": f"https://httpbin.org/json", "format": "json"},
        {"url": f"https://httpbin.org/html", "format": "text"},
        {"url": f"https://httpbin.org/xml", "format": "text"},
    ] * 5  # 15 URLs total
...
async def main():
    ...
    try:
        ...
        print(f"\n🎯 Examples Summary:")
        print(f"   All batch processing examples completed!")
        print(f"   Check the '{OUTPUT_DIR}' directory for saved results.")
        print(f"   Each batch creates both detailed JSON logs and individual content files.")
        
        print(f"\n💡 Tips for using batch processing:")
        print(f"   • Use appropriate concurrency limits based on your server resources")
        print(f"   • Set reasonable timeouts for different content types (PDF takes longer)")
        print(f"   • Monitor the batch_id for tracking specific batch operations")
        print(f"   • Handle partial failures gracefully - some URLs may fail while others succeed")
        print(f"   • Use different formats in the same batch for efficient multi-format processing")
