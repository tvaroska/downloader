...
This example demonstrates the fundamental usage patterns of the REST API Downloader service.
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Success! Received structured JSON response")
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"⏳ PDF service temporarily unavailable (503)")
                print(f"   This is normal under high load - the service limits concurrent PDF generation")
    ...
    async with httpx.AsyncClient() as client:
        for url in invalid_urls:
            ...
            try:
                ...
                if response.status_code == 200:
                    print(f"   ✅ Unexpected success (this shouldn't happen)")
                else:
...
async def main():
    ...
    try:
        ...
        print(f"\n💡 Key takeaways:")
        print(f"   • Use Accept headers to control response format")
        print(f"   • Check response headers for metadata")
        print(f"   • Handle errors gracefully with proper status codes")
        print(f"   • PDF generation may have delays under load")
        print(f"\n📚 Next steps:")
        print(f"   • Try concurrent_pdf_requests.py for load testing")
        print(f"   • Explore content_formats.py for format comparison")
        print(f"   • Check error_handling.py for robust error patterns")
